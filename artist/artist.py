...
class artist():
	"""
	This is the object responsible for drawing a layout from parsed html + parsed css.

	Ex:
	---
	artist = strange.artist()
	artist.drawGDS(htmlDoc, stylesheet)
	"""
	...
	def drawGDS(htmlDoc):
		...
		if len(techElt) != 1:
			except("FATAL: Exactly 1 techfile must be specified in the document's header.")
			return
		if 'file' not in techElt[0].attrib:
			except("FATAL: No 'file' attribute of <tech> element given.")
			return
	...
	def _parseTechFile(fileName):
		...
		for rule in stylesheet.rules:
			# Decode units/precision rule / skip any others
			if rule.at_keyword == 'page':
				thisDecl = self._declaration2dict(rule.declarations)
				this.units 		= (	thisDecl['units'].value *
									self.unitsSI[ thisDecl['units'].unit[0] ] )
				this.precision 	= (	thisDecl['precision'].value *
									self.unitsSI[ thisDecl['precision'].unit[0] ] )
				continue
			elif rule.at_keyword != None:
				continue
			...
			try:
				...
				raise("FATAL: techfile is malformed at line " + str(rule.line))

			deviceMap[thisTag] = getattr(globs[thisLib],thisDev)

			thisDecl.pop('libName')
			thisDecl.pop('devName')
			defaultAttribs[thisTag] = thisDecl
	...
	def _parseStylesheets(ssFileNames, htmlBody):
		"""
		...
		tagParams = idParams = classParams = []
		for p in params:
			tagParams 	+= p[0]
			idParams 	+= p[1]
			classParams += p[2]
	...
	def _parseStylesheet(fileName, htmlBody):
		"""
		...
		for rule in stylesheet.rules:
			...
			thisDecl = self._declaration2dict(rule.declarations)
## FINISH ME ##
			## Decode selector types - how does this work?
			# thisSel  = rule.selector

			## Do the easy selectors as a dictionary
			# out[thisSel] = thisDecl

			## Do the hard selectors as in-line style
			# cssselect(thisSel).addAttrib('style', rule.declarations)

		return out


	def _declaration2dict(declarations):
		"""
		...
		for decl in declarations:
			thisValue = self._parseValue(decl.value[0])
			thisProp  = self._parseProp(decl.value)
	...
	def _parseProp(name, value):
		...
		valueLen = len(value)
		parsedVals = [self._parseValue(x) for x in value]
		...
		if name in simple_prefixes:
			if valueLen == 1:
				...
				raise("""Incorrect # of values supplied to a shorthand CSS expression.
					  Only 1-value and 4-value syntaxes are supported.
					  (On line """ + value[0].line + ", column " + value[0].column )
			...
			if valueLen != 1:
				raise("""Incorrect # of values supplied to a CSS expression.
					  (On line """ + value[0].line + ", column " + value[0].column )
			return parsedVal[0]
	...
	def _parseValue(value):
		...
		if value.type == 'DIMENSION':
			if value.unit == 'px':
				return value.value * self.precision
			else:
			...
			raise("Illegal value in css at line " + value.line +
									  ", column " + value.column )


	def _combineParams(paramsList):
		"""
